...
class CqustCardSystemMysql(BaseMysql):
    ...
    def is_sno_exist(self, sno):
        ...
        """
        try:
            is_sno_exist = self.base_is_attribute_exist_by_table(table_name=MYSQL_RFID_INFO_TABLE, attribute_name="sno",
                                                                 attribute_value=sno)
            if is_sno_exist:
                return True
            else:
    ...
    def is_card_exist(self, card_id):
        ...
        """
        try:
            is_card_exist = self.base_is_attribute_exist_by_table(table_name=MYSQL_RFID_INFO_TABLE,
                                                                  attribute_name="card_id", attribute_value=card_id)
            if is_card_exist:
                return True
            else:
                return False
    ...
    def get_sno_by_card_id_from_rfid_info(self, card_id):
        ...
        """
        try:
            result = self.base_get_attribute_value_by_table_and_one_condition(table_name=MYSQL_RFID_INFO_TABLE,
                                                                              attribute_name="sno",
                                                                              condition_name="card_id",
                                                                              condition_value=card_id)
            return result[0][0]
        except Exception as e:
    ...
    def get_card_id_by_sno_from_rfid_info(self, sno):
        ...
        try:
            result = self.base_get_attribute_value_by_table_and_one_condition(table_name=MYSQL_RFID_INFO_TABLE,
                                                                              attribute_name="card_id",
                                                                              condition_name="sno", condition_value=sno)
            return result[0][0]
        except Exception as e:
    ...
    def get_is_user_exist_by_user_info(self, user_id):
        ...
        """
        try:
            result = self.base_is_attribute_exist_by_table(table_name=MYSQL_USER_INFO_TABLE, attribute_name="user_id",
    ...
    def set_user_state(self, user_id, state):
        ...
        :return:
        """
        return self.base_update_attribute_value_by_table_and_one_condition(table_name=MYSQL_USER_INFO_TABLE,
                                                                           attribute_name="state",
                                                                           attribute_value=state,
                                                                           condition_value=user_id,
                                                                           condition_name="user_id")
    ...
    def get_user_state(self, user_id):
        ...
        :return:
        """
        result = self.base_get_attribute_value_by_table_and_one_condition(table_name=MYSQL_USER_INFO_TABLE,
                                                                          attribute_name="state",
                                                                          condition_name="user_id",
                                                                          condition_value=user_id)
        if result:
    ...
    def get_user_name_by_user_id(self, user_id):
        ...
        result = self.base_get_attribute_value_by_table_and_one_condition(table_name=MYSQL_TEACHER_INFO_TABLE,
                                                                          attribute_name="teacher_name",
                                                                          condition_name="teacher_id",
                                                                          condition_value=user_id)
        if result:
            return result[0][0]
        else:
    ...
    def get_user_school_id_by_user_id(self, user_id):
        ...
        """
        result = self.base_get_attribute_value_by_table_and_one_condition(table_name=MYSQL_TEACHER_INFO_TABLE,
                                                                          attribute_name="teacher_school_id",
                                                                          condition_name="teacher_id",
                                                                          condition_value=user_id)
        if result:
            return result[0][0]
    ...
    def get_stu_name_by_sno(self, sno):
        ...
        :return:
        """
        result = self.base_get_attribute_value_by_table_and_one_condition(attribute_name="name",
                                                                          table_name=MYSQL_STU_INFO_TABLE,
                                                                          condition_name="sno", condition_value=sno)
        if result:
            return result[0][0]
        else:
    ...
    def add_attendence_to_attendence_info(self, addtime, sno, course_sections, scheduling_id, state, place):
        ...
        """
        try:
            insert_data = {"addtime": addtime, "sno": sno, 'course_sections': course_sections,
                           'scheduling_id': scheduling_id, 'state': state, 'place': place}
            result = self.base_insert_value_to_table(table_name=MYSQL_ATTENDENCE_INFO_TABLE, insert_data=insert_data)
            return result
        except Exception as e:
            print("错误来自于 add_attendence_to_attendence_info， 参数{} {} ".format(addtime, sno, str(course_sections),
                                                                                    scheduling_id, str(state),
                                                                                    place) + e)
...
and scheduling_id=%s
    ...
    def get_teacher_email_by_teacher_name(self, teacher_name):
        result = self.base_get_attribute_value_by_table_and_one_condition(table_name=MYSQL_TEACHER_INFO_TABLE,
                                                                          attribute_name="teacher_email",
                                                                          condition_name="teacher_name",
                                                                          condition_value=teacher_name)
        return result[0][0]
