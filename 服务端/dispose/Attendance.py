...
class Attendance:
    ...
    def __add_attendance_info(cls, sno, course_sections, scheduling_id, state, place):
        ...
        result = mysql_cqust_rfid.add_attendence_to_attendence_info(addtim, sno, course_sections, scheduling_id, state,
                                                                    place)
        return result
    ...
    def attendance(cls, sno, time_str, place):
        ...
        for i in stu_courses:
            # 节次 地点 课程ID 星期几
            result = mysql_cqust_rfid.get_techer_id_and_scheduling_id_by_time_and_course_id_and_place(
                course_section=course_section['section'], course_id=i, place=place)
            if result:
                start_time = (datetime.combine(datetime.today(), datetime.strptime(
                    time.strftime('%Y-%m-%d ', time.localtime()) + COURSE_TIMES[course_section['section'] - 1][0],
                    "%Y-%m-%d %H:%M:%S").time()) - timedelta(
                    minutes=COURSE_AGAIN_TIME))
                end_time = (datetime.combine(datetime.today(), datetime.strptime(
                    time.strftime('%Y-%m-%d ', time.localtime()) + COURSE_TIMES[course_section['section'] - 1][1],
                    "%Y-%m-%d %H:%M:%S").time()) + timedelta(
                ...
                res = cls.__add_attendance_info(sno, course_section['section'], result['scheduling_id'],
                                                course_section['flag'], place)
                if res:
